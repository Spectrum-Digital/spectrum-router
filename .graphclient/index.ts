...
import * as importedModule$0 from "./sources/SpectrumRouterBase/introspectionSchema";
...
export type PoolFactory_orderBy =
  | 'id'
  | 'pools';
...
const additionalTypeDefs = [parse("extend type Pool {\n  chainId: String!\n}"),] as any[];
...
export type PoolsQuery = { pools: Array<(
    Pick<Pool, 'blockNumber' | 'id' | 'stable'>
    & { token0: Pick<Token, 'id' | 'decimals'>, token1: Pick<Token, 'id' | 'decimals'> }
  )> };
